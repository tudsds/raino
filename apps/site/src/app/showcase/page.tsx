      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className={isEven ? '' : 'lg:order-2'}>
            ...
            <p className="text-[#a1a1aa] mb-8">{project.description}</p>

            <div className="grid grid-cols-2 gap-4 mb-8">
              {project.specs.map((spec) => (
                <div key={spec} className="flex items-center gap-2 text-sm text-[#a1a1aa]">
            ...
            <div className="p-4 bg-[#1a1a24] border-2 border-[#27272a]">
              <h4 className="text-sm font-semibold text-[#71717a] mb-3">Raino Deliverables</h4>
              <ul className="space-y-2">
                {project.deliverables.map((d) => (
                  <li key={d} className="flex items-center gap-2 text-sm text-[#a1a1aa]">
          ...
          <div className={`${isEven ? '' : 'lg:order-1'}`}>
            <div className="aspect-video bg-[#0a0a0f] border-2 border-[#27272a] p-6 flex items-center justify-center relative overflow-hidden">
              ...
              <div className="relative z-10 w-full max-w-sm">
                ...
                >
                  <div className="absolute top-2 left-2 right-2 bottom-2">
                    <div
                      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 border-2"
                      style={{ borderColor: `${project.color}60` }}
                    />
                    <div
                      className="absolute top-2 left-1/2 -translate-x-1/2 w-8 h-3 border-2"
                      style={{ borderColor: `${project.color}40` }}
                    />
                    <div
                      className="absolute bottom-2 left-1/2 -translate-x-1/2 w-8 h-3 border-2"
                      style={{ borderColor: `${project.color}40` }}
                    />
                    <div
                      className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-8 border-2"
                      style={{ borderColor: `${project.color}40` }}
                    />
                    <div
      ...
      <section className="py-24 bg-[#0a0a0f] circuit-grid">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold font-[family-name:var(--font-heading)] mb-6">
            Projects Built with <span className="gradient-text">Raino</span>
          </h1>
      ...
      <section className="py-24 bg-[#111118]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold font-[family-name:var(--font-heading)] mb-6">
            Ready to Build <span className="gradient-text">Your Project?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              href="/workflow"
              className="px-8 py-4 bg-[#0a0a0f] border-2 border-[#27272a] text-[#e4e4e7] font-semibold hover:border-[#00f0ff] hover:text-[#00f0ff] transition-all duration-300"
            >
              See the Workflow
            </Link>
          </div>
