...
export default function Navbar({ activePath }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[rgba(10,25,41,0.80)] backdrop-blur-[20px] backdrop-saturate-[160%] border-b border-[rgba(97,145,211,0.3)]">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 text-base">
              {languages.map((lang) => (
                <span key={lang.code} className="glass-surface px-2 py-1 text-xs">
                  {lang.code === 'en' ? (
      ...
      {isOpen && (
        <div
          ref={menuRef}
          className="md:hidden absolute top-16 left-0 right-0 glass-elevated border-t-0"
        >
