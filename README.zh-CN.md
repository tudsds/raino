...
平台包含两个前端应用、八个共享包和四个 worker 服务。使用 Supabase 做持久化和认证，Kimi K2.5 做 LLM 推理，像素艺术赛博朋克设计系统。全部代码基于 MIT 许可证开源。
...
│   ├── agents/              # 工作流合约、状态机、编排
│   ├── ui/                  # 像素艺术赛博朋克设计系统（React + Tailwind v4）
...
| 设计   | @raino/ui（像素艺术赛博朋克, Press Start 2P + VT323） |
...
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | Supabase 匿名密钥     | 否（降级模式）     |
