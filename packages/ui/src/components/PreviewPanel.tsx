    ...
    <Card variant="elevated" className="overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#27272a]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-[#e4e4e7]">{typeLabels[type]}</span>
        </div>
      </div>
      ...
      <div className="relative aspect-video bg-[#0a0a0f] flex items-center justify-center">
        {loading && (
          <div className="flex flex-col items-center gap-3">
            <div className="w-8 h-8 border-2 border-[#27272a] border-t-[#00f0ff] rounded-full animate-spin" />
            <span className="text-sm text-[#71717a]">Loading preview...</span>
          </div>
        )}
        ...
        {modelUrl && !loading && !error && type === 'pcb3d' && (
          <div className="flex flex-col items-center gap-2">
            <span className="text-sm text-[#00f0ff]">3D Model Ready</span>
            <span className="text-xs text-[#71717a]">{modelUrl}</span>
          </div>
        )}
      </div>
    </Card>
